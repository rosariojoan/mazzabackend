...
func (tc *TokenCreate) check() error {
	if _, ok := tc.mutation.Expiry(); !ok {
		return &ValidationError{Name: "expiry", err: errors.New(`generated: missing required field "Token.expiry"`)}
	}
	if _, ok := tc.mutation.Category(); !ok {
		return &ValidationError{Name: "category", err: errors.New(`generated: missing required field "Token.category"`)}
	}
	if v, ok := tc.mutation.Category(); ok {
		if err := token.CategoryValidator(v); err != nil {
			return &ValidationError{Name: "category", err: fmt.Errorf(`generated: validator failed for field "Token.category": %w`, err)}
		}
	}
	if _, ok := tc.mutation.Token(); !ok {
		return &ValidationError{Name: "token", err: errors.New(`generated: missing required field "Token.token"`)}
	}
	if v, ok := tc.mutation.Token(); ok {
		if err := token.TokenValidator(v); err != nil {
			return &ValidationError{Name: "token", err: fmt.Errorf(`generated: validator failed for field "Token.token": %w`, err)}
		}
	}
	return nil
}
