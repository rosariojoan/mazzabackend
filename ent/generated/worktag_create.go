...
func (wc *WorktagCreate) check() error {
	if _, ok := wc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "createdAt", err: errors.New(`generated: missing required field "Worktag.createdAt"`)}
	}
	if _, ok := wc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updatedAt", err: errors.New(`generated: missing required field "Worktag.updatedAt"`)}
	}
	if _, ok := wc.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`generated: missing required field "Worktag.name"`)}
	}
	if v, ok := wc.mutation.Name(); ok {
		if err := worktag.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`generated: validator failed for field "Worktag.name": %w`, err)}
		}
	}
	if _, ok := wc.mutation.Color(); !ok {
		return &ValidationError{Name: "color", err: errors.New(`generated: missing required field "Worktag.color"`)}
	}
	return nil
}
