...
func (tq *TokenQuery) QueryCompany() *CompanyQuery {
...
func (tq *TokenQuery) WithCompany(opts ...func(*CompanyQuery)) *TokenQuery {
	query := (&CompanyClient{config: tq.config}).Query()
...
func (tq *TokenQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Token, error) {
	...
	if tq.withCompany != nil || tq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withCompany; query != nil {
		if err := tq.loadCompany(ctx, query, nodes, nil,
			func(n *Token, e *Company) { n.Edges.Company = e }); err != nil {
			return nil, err
		}
	}
...
func (tq *TokenQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*Token, init func(*Token), assign func(*Token, *Company)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Token)
	for i := range nodes {
		if nodes[i].company_tokens == nil {
			continue
		}
		fk := *nodes[i].company_tokens
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "company_tokens" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
