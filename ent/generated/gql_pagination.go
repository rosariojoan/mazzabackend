...
// CashMovementEdge is the edge representation of CashMovement.
type CashMovementEdge struct {
	Node   *CashMovement `json:"node"`
	Cursor Cursor        `json:"cursor"`
}

// CashMovementConnection is the connection containing edges to CashMovement.
type CashMovementConnection struct {
	Edges      []*CashMovementEdge `json:"edges"`
	PageInfo   PageInfo            `json:"pageInfo"`
	TotalCount int                 `json:"totalCount"`
}

func (c *CashMovementConnection) build(nodes []*CashMovement, pager *cashmovementPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *CashMovement
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *CashMovement {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *CashMovement {
	...
	c.Edges = make([]*CashMovementEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &CashMovementEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// CashMovementPaginateOption enables pagination customization.
type CashMovementPaginateOption func(*cashmovementPager) error

// WithCashMovementOrder configures pagination ordering.
func WithCashMovementOrder(order *CashMovementOrder) CashMovementPaginateOption {
	if order == nil {
		order = DefaultCashMovementOrder
	}
	o := *order
	return func(pager *cashmovementPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultCashMovementOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithCashMovementFilter configures pagination filter.
func WithCashMovementFilter(filter func(*CashMovementQuery) (*CashMovementQuery, error)) CashMovementPaginateOption {
	return func(pager *cashmovementPager) error {
		if filter == nil {
			return errors.New("CashMovementQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type cashmovementPager struct {
	reverse bool
	order   *CashMovementOrder
	filter  func(*CashMovementQuery) (*CashMovementQuery, error)
}

func newCashMovementPager(opts []CashMovementPaginateOption, reverse bool) (*cashmovementPager, error) {
	pager := &cashmovementPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultCashMovementOrder
	}
	return pager, nil
}

func (p *cashmovementPager) applyFilter(query *CashMovementQuery) (*CashMovementQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *cashmovementPager) toCursor(cm *CashMovement) Cursor {
	return p.order.Field.toCursor(cm)
}

func (p *cashmovementPager) applyCursors(query *CashMovementQuery, after, before *Cursor) (*CashMovementQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultCashMovementOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *cashmovementPager) applyOrder(query *CashMovementQuery) *CashMovementQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultCashMovementOrder.Field {
		query = query.Order(DefaultCashMovementOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *cashmovementPager) orderExpr(query *CashMovementQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultCashMovementOrder.Field {
			b.Comma().Ident(DefaultCashMovementOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to CashMovement.
func (cm *CashMovementQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...CashMovementPaginateOption,
) (*CashMovementConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newCashMovementPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	...
	conn := &CashMovementConnection{Edges: []*CashMovementEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) || hasCollectedField(ctx, pageInfoField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
...
// CashMovementOrderField defines the ordering field of CashMovement.
type CashMovementOrderField struct {
	// Value extracts the ordering value from the given CashMovement.
	Value    func(*CashMovement) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) cashmovement.OrderOption
	toCursor func(*CashMovement) Cursor
}

// CashMovementOrder defines the ordering of CashMovement.
type CashMovementOrder struct {
	Direction OrderDirection          `json:"direction"`
	Field     *CashMovementOrderField `json:"field"`
}

// DefaultCashMovementOrder is the default ordering of CashMovement.
var DefaultCashMovementOrder = &CashMovementOrder{
	Direction: entgql.OrderDirectionAsc,
	Field: &CashMovementOrderField{
		Value: func(cm *CashMovement) (ent.Value, error) {
			return cm.ID, nil
		},
		column: cashmovement.FieldID,
		toTerm: cashmovement.ByID,
		toCursor: func(cm *CashMovement) Cursor {
			return Cursor{ID: cm.ID}
		},
	},
}

// ToEdge converts CashMovement into CashMovementEdge.
func (cm *CashMovement) ToEdge(order *CashMovementOrder) *CashMovementEdge {
	if order == nil {
		order = DefaultCashMovementOrder
	}
	return &CashMovementEdge{
		Node:   cm,
...
// ProductMovementEdge is the edge representation of ProductMovement.
type ProductMovementEdge struct {
	Node   *ProductMovement `json:"node"`
	Cursor Cursor           `json:"cursor"`
}

// ProductMovementConnection is the connection containing edges to ProductMovement.
type ProductMovementConnection struct {
	Edges      []*ProductMovementEdge `json:"edges"`
	PageInfo   PageInfo               `json:"pageInfo"`
	TotalCount int                    `json:"totalCount"`
}

func (c *ProductMovementConnection) build(nodes []*ProductMovement, pager *productmovementPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *ProductMovement
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *ProductMovement {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *ProductMovement {
	...
	c.Edges = make([]*ProductMovementEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &ProductMovementEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// ProductMovementPaginateOption enables pagination customization.
type ProductMovementPaginateOption func(*productmovementPager) error

// WithProductMovementOrder configures pagination ordering.
func WithProductMovementOrder(order *ProductMovementOrder) ProductMovementPaginateOption {
	if order == nil {
		order = DefaultProductMovementOrder
	}
	o := *order
	return func(pager *productmovementPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultProductMovementOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithProductMovementFilter configures pagination filter.
func WithProductMovementFilter(filter func(*ProductMovementQuery) (*ProductMovementQuery, error)) ProductMovementPaginateOption {
	return func(pager *productmovementPager) error {
		if filter == nil {
			return errors.New("ProductMovementQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type productmovementPager struct {
	reverse bool
	order   *ProductMovementOrder
	filter  func(*ProductMovementQuery) (*ProductMovementQuery, error)
}

func newProductMovementPager(opts []ProductMovementPaginateOption, reverse bool) (*productmovementPager, error) {
	pager := &productmovementPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultProductMovementOrder
	}
	return pager, nil
}

func (p *productmovementPager) applyFilter(query *ProductMovementQuery) (*ProductMovementQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *productmovementPager) toCursor(pm *ProductMovement) Cursor {
	return p.order.Field.toCursor(pm)
}

func (p *productmovementPager) applyCursors(query *ProductMovementQuery, after, before *Cursor) (*ProductMovementQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultProductMovementOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *productmovementPager) applyOrder(query *ProductMovementQuery) *ProductMovementQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultProductMovementOrder.Field {
		query = query.Order(DefaultProductMovementOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *productmovementPager) orderExpr(query *ProductMovementQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultProductMovementOrder.Field {
			b.Comma().Ident(DefaultProductMovementOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to ProductMovement.
func (pm *ProductMovementQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...ProductMovementPaginateOption,
) (*ProductMovementConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newProductMovementPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	...
	conn := &ProductMovementConnection{Edges: []*ProductMovementEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) || hasCollectedField(ctx, pageInfoField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
...
// ProductMovementOrderField defines the ordering field of ProductMovement.
type ProductMovementOrderField struct {
	// Value extracts the ordering value from the given ProductMovement.
	Value    func(*ProductMovement) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) productmovement.OrderOption
	toCursor func(*ProductMovement) Cursor
}

// ProductMovementOrder defines the ordering of ProductMovement.
type ProductMovementOrder struct {
	Direction OrderDirection             `json:"direction"`
	Field     *ProductMovementOrderField `json:"field"`
}

// DefaultProductMovementOrder is the default ordering of ProductMovement.
var DefaultProductMovementOrder = &ProductMovementOrder{
	Direction: entgql.OrderDirectionAsc,
	Field: &ProductMovementOrderField{
		Value: func(pm *ProductMovement) (ent.Value, error) {
			return pm.ID, nil
		},
		column: productmovement.FieldID,
		toTerm: productmovement.ByID,
		toCursor: func(pm *ProductMovement) Cursor {
			return Cursor{ID: pm.ID}
		},
	},
}

// ToEdge converts ProductMovement into ProductMovementEdge.
func (pm *ProductMovement) ToEdge(order *ProductMovementOrder) *ProductMovementEdge {
	if order == nil {
		order = DefaultProductMovementOrder
	}
	return &ProductMovementEdge{
		Node:   pm,
...
// WorktagEdge is the edge representation of Worktag.
type WorktagEdge struct {
	Node   *Worktag `json:"node"`
	Cursor Cursor   `json:"cursor"`
}

// WorktagConnection is the connection containing edges to Worktag.
type WorktagConnection struct {
	Edges      []*WorktagEdge `json:"edges"`
	PageInfo   PageInfo       `json:"pageInfo"`
	TotalCount int            `json:"totalCount"`
}

func (c *WorktagConnection) build(nodes []*Worktag, pager *worktagPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *Worktag
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *Worktag {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *Worktag {
	...
	c.Edges = make([]*WorktagEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &WorktagEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// WorktagPaginateOption enables pagination customization.
type WorktagPaginateOption func(*worktagPager) error

// WithWorktagOrder configures pagination ordering.
func WithWorktagOrder(order *WorktagOrder) WorktagPaginateOption {
	if order == nil {
		order = DefaultWorktagOrder
	}
	o := *order
	return func(pager *worktagPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultWorktagOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithWorktagFilter configures pagination filter.
func WithWorktagFilter(filter func(*WorktagQuery) (*WorktagQuery, error)) WorktagPaginateOption {
	return func(pager *worktagPager) error {
		if filter == nil {
			return errors.New("WorktagQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type worktagPager struct {
	reverse bool
	order   *WorktagOrder
	filter  func(*WorktagQuery) (*WorktagQuery, error)
}

func newWorktagPager(opts []WorktagPaginateOption, reverse bool) (*worktagPager, error) {
	pager := &worktagPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultWorktagOrder
	}
	return pager, nil
}

func (p *worktagPager) applyFilter(query *WorktagQuery) (*WorktagQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *worktagPager) toCursor(w *Worktag) Cursor {
	return p.order.Field.toCursor(w)
}

func (p *worktagPager) applyCursors(query *WorktagQuery, after, before *Cursor) (*WorktagQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultWorktagOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *worktagPager) applyOrder(query *WorktagQuery) *WorktagQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultWorktagOrder.Field {
		query = query.Order(DefaultWorktagOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *worktagPager) orderExpr(query *WorktagQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultWorktagOrder.Field {
			b.Comma().Ident(DefaultWorktagOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to Worktag.
func (w *WorktagQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...WorktagPaginateOption,
) (*WorktagConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newWorktagPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	if w, err = pager.applyFilter(w); err != nil {
	...
	conn := &WorktagConnection{Edges: []*WorktagEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) || hasCollectedField(ctx, pageInfoField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
...
// WorktagOrderField defines the ordering field of Worktag.
type WorktagOrderField struct {
	// Value extracts the ordering value from the given Worktag.
	Value    func(*Worktag) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) worktag.OrderOption
	toCursor func(*Worktag) Cursor
}

// WorktagOrder defines the ordering of Worktag.
type WorktagOrder struct {
	Direction OrderDirection     `json:"direction"`
	Field     *WorktagOrderField `json:"field"`
}

// DefaultWorktagOrder is the default ordering of Worktag.
var DefaultWorktagOrder = &WorktagOrder{
	Direction: entgql.OrderDirectionAsc,
	Field: &WorktagOrderField{
		Value: func(w *Worktag) (ent.Value, error) {
			return w.ID, nil
		},
		column: worktag.FieldID,
		toTerm: worktag.ByID,
		toCursor: func(w *Worktag) Cursor {
			return Cursor{ID: w.ID}
		},
	},
}

// ToEdge converts Worktag into WorktagEdge.
func (w *Worktag) ToEdge(order *WorktagOrder) *WorktagEdge {
	if order == nil {
		order = DefaultWorktagOrder
	}
	return &WorktagEdge{
...
// WorktaskEdge is the edge representation of Worktask.
type WorktaskEdge struct {
	Node   *Worktask `json:"node"`
	Cursor Cursor    `json:"cursor"`
}

// WorktaskConnection is the connection containing edges to Worktask.
type WorktaskConnection struct {
	Edges      []*WorktaskEdge `json:"edges"`
	PageInfo   PageInfo        `json:"pageInfo"`
	TotalCount int             `json:"totalCount"`
}

func (c *WorktaskConnection) build(nodes []*Worktask, pager *worktaskPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *Worktask
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *Worktask {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *Worktask {
	...
	c.Edges = make([]*WorktaskEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &WorktaskEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// WorktaskPaginateOption enables pagination customization.
type WorktaskPaginateOption func(*worktaskPager) error

// WithWorktaskOrder configures pagination ordering.
func WithWorktaskOrder(order *WorktaskOrder) WorktaskPaginateOption {
	if order == nil {
		order = DefaultWorktaskOrder
	}
	o := *order
	return func(pager *worktaskPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultWorktaskOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithWorktaskFilter configures pagination filter.
func WithWorktaskFilter(filter func(*WorktaskQuery) (*WorktaskQuery, error)) WorktaskPaginateOption {
	return func(pager *worktaskPager) error {
		if filter == nil {
			return errors.New("WorktaskQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type worktaskPager struct {
	reverse bool
	order   *WorktaskOrder
	filter  func(*WorktaskQuery) (*WorktaskQuery, error)
}

func newWorktaskPager(opts []WorktaskPaginateOption, reverse bool) (*worktaskPager, error) {
	pager := &worktaskPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultWorktaskOrder
	}
	return pager, nil
}

func (p *worktaskPager) applyFilter(query *WorktaskQuery) (*WorktaskQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *worktaskPager) toCursor(w *Worktask) Cursor {
	return p.order.Field.toCursor(w)
}

func (p *worktaskPager) applyCursors(query *WorktaskQuery, after, before *Cursor) (*WorktaskQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultWorktaskOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *worktaskPager) applyOrder(query *WorktaskQuery) *WorktaskQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultWorktaskOrder.Field {
		query = query.Order(DefaultWorktaskOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *worktaskPager) orderExpr(query *WorktaskQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultWorktaskOrder.Field {
			b.Comma().Ident(DefaultWorktaskOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to Worktask.
func (w *WorktaskQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...WorktaskPaginateOption,
) (*WorktaskConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newWorktaskPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	if w, err = pager.applyFilter(w); err != nil {
	...
	conn := &WorktaskConnection{Edges: []*WorktaskEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) || hasCollectedField(ctx, pageInfoField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
...
// WorktaskOrderField defines the ordering field of Worktask.
type WorktaskOrderField struct {
	// Value extracts the ordering value from the given Worktask.
	Value    func(*Worktask) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) worktask.OrderOption
	toCursor func(*Worktask) Cursor
}

// WorktaskOrder defines the ordering of Worktask.
type WorktaskOrder struct {
	Direction OrderDirection      `json:"direction"`
	Field     *WorktaskOrderField `json:"field"`
}

// DefaultWorktaskOrder is the default ordering of Worktask.
var DefaultWorktaskOrder = &WorktaskOrder{
	Direction: entgql.OrderDirectionAsc,
	Field: &WorktaskOrderField{
		Value: func(w *Worktask) (ent.Value, error) {
			return w.ID, nil
		},
		column: worktask.FieldID,
		toTerm: worktask.ByID,
		toCursor: func(w *Worktask) Cursor {
			return Cursor{ID: w.ID}
		},
	},
}

// ToEdge converts Worktask into WorktaskEdge.
func (w *Worktask) ToEdge(order *WorktaskOrder) *WorktaskEdge {
	if order == nil {
		order = DefaultWorktaskOrder
	}
	return &WorktaskEdge{
