...
func (e TokenEdges) CompanyOrErr() (*Company, error) {
	if e.Company != nil {
...
func (*Token) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case token.FieldCategory, token.FieldToken:
			values[i] = new(sql.NullString)
		case token.FieldExpiry:
			values[i] = new(sql.NullTime)
		case token.ForeignKeys[0]: // company_tokens
			values[i] = new(sql.NullInt64)
	...
	for i := range columns {
		...
		case token.FieldCategory:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				t.Category = token.Category(value.String)
			}
		...
		case token.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field company_tokens", value)
			} else if value.Valid {
				t.company_tokens = new(int)
				*t.company_tokens = int(value.Int64)
			}
...
// QueryCompany queries the "company" edge of the Token entity.
func (t *Token) QueryCompany() *CompanyQuery {
	return NewTokenClient(t.config).QueryCompany(t)
}
...
func (t *Token) Unwrap() *Token {
	...
	if !ok {
		panic("generated: Token is not a transactional entity")
	}
