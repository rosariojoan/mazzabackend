...
func (fq *FileQuery) QueryCompany() *CompanyQuery {
	query := (&CompanyClient{config: fq.config}).Query()
...
func (fq *FileQuery) WithCompany(opts ...func(*CompanyQuery)) *FileQuery {
	query := (&CompanyClient{config: fq.config}).Query()
...
func (fq *FileQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*File, init func(*File), assign func(*File, *Company)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*File)
	for i := range nodes {
		if nodes[i].company_files == nil {
			continue
		}
		fk := *nodes[i].company_files
