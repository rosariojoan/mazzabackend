...
func (urq *UserRoleQuery) QueryCompany() *CompanyQuery {
	query := (&CompanyClient{config: urq.config}).Query()
...
func (urq *UserRoleQuery) WithCompany(opts ...func(*CompanyQuery)) *UserRoleQuery {
	query := (&CompanyClient{config: urq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	urq.withCompany = query
	return urq
}
...
func (urq *UserRoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserRole, error) {
	...
	if query := urq.withCompany; query != nil {
		if err := urq.loadCompany(ctx, query, nodes, nil,
			func(n *UserRole, e *Company) { n.Edges.Company = e }); err != nil {
			return nil, err
		}
	}
...
func (urq *UserRoleQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*UserRole, init func(*UserRole), assign func(*UserRole, *Company)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*UserRole)
	for i := range nodes {
		if nodes[i].company_available_roles == nil {
			continue
		}
		fk := *nodes[i].company_available_roles
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "company_available_roles" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
