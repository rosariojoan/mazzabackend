...
func (pc *PayableCreate) check() error {
	if _, ok := pc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "createdAt", err: errors.New(`ent: missing required field "Payable.createdAt"`)}
	}
	if _, ok := pc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updatedAt", err: errors.New(`ent: missing required field "Payable.updatedAt"`)}
	}
	if _, ok := pc.mutation.EntryGroup(); !ok {
		return &ValidationError{Name: "entryGroup", err: errors.New(`ent: missing required field "Payable.entryGroup"`)}
	}
	if v, ok := pc.mutation.EntryGroup(); ok {
		if err := payable.EntryGroupValidator(v); err != nil {
			return &ValidationError{Name: "entryGroup", err: fmt.Errorf(`ent: validator failed for field "Payable.entryGroup": %w`, err)}
		}
	}
	if _, ok := pc.mutation.Date(); !ok {
		return &ValidationError{Name: "date", err: errors.New(`ent: missing required field "Payable.date"`)}
	}
	if _, ok := pc.mutation.OutstandingBalance(); !ok {
		return &ValidationError{Name: "outstandingBalance", err: errors.New(`ent: missing required field "Payable.outstandingBalance"`)}
	}
	if _, ok := pc.mutation.TotalTransaction(); !ok {
		return &ValidationError{Name: "totalTransaction", err: errors.New(`ent: missing required field "Payable.totalTransaction"`)}
	}
	if _, ok := pc.mutation.DaysDue(); !ok {
		return &ValidationError{Name: "daysDue", err: errors.New(`ent: missing required field "Payable.daysDue"`)}
	}
	if v, ok := pc.mutation.DaysDue(); ok {
		if err := payable.DaysDueValidator(v); err != nil {
			return &ValidationError{Name: "daysDue", err: fmt.Errorf(`ent: validator failed for field "Payable.daysDue": %w`, err)}
		}
	}
	if _, ok := pc.mutation.Status(); !ok {
		return &ValidationError{Name: "status", err: errors.New(`ent: missing required field "Payable.status"`)}
	}
