...
func (cc *CustomerCreate) check() error {
	...
	if _, ok := cc.mutation.Description(); !ok {
		return &ValidationError{Name: "description", err: errors.New(`ent: missing required field "Customer.description"`)}
	}
	...
	if _, ok := cc.mutation.TaxId(); !ok {
		return &ValidationError{Name: "taxId", err: errors.New(`ent: missing required field "Customer.taxId"`)}
	}
	if v, ok := cc.mutation.TaxId(); ok {
		if err := customer.TaxIdValidator(v); err != nil {
			return &ValidationError{Name: "taxId", err: fmt.Errorf(`ent: validator failed for field "Customer.taxId": %w`, err)}
		}
	}
	return nil
}
