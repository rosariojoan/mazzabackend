...
func SendDataNotification(FCMToken string, title *string, data map[string]string) {
	resp, err := inits.FCM.Send(context.Background(), &messaging.Message{
	...
	fmt.Printf("Nofit sent successful: %s\n", resp)
}

// Convert struct to map to be sent sent as notification data
func GetMap(data interface{}) (map[string]string, error) {
