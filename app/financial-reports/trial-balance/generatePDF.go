...
func generatePDFDoc(data []*model.TrialBalanceRowItem, company *ent.Company, startDate time.Time, endDate time.Time) (file *string, size float64, err error) {
	...
	tableHeader := []string{"Conta", "Nome", "Débito", "Crédito", "Saldo"}
	columnWidths := []int{2, 4, 2, 2, 2}
	keywords := "trial balance,balancete"
	subject := "trial balance/balancete"
...
func getTableRows(data []*model.TrialBalanceRowItem) (body [][]string, footer []string) {
	totalDebit, totalCredit := float64(0), float64(0)

	for _, entry := range data {
		debit, credit, balance := utils.ToCurrencyStr(entry.Debit), utils.ToCurrencyStr(entry.Credit), utils.ToCurrencyStr(entry.Balance)

		// if accountingentry.IsDebitable(entry.AccountType) && entry.Credit < 0 {
		// 	credit = utils.ToCurrencyStr(-entry.Credit)
		// } else if !accountingentry.IsDebitable(entry.AccountType) && entry.Debit < 0 {
		// 	debit = utils.ToCurrencyStr(-entry.Debit)
		// }

		row := []string{entry.Account, entry.Label, debit, credit, balance}
