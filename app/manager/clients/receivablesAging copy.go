...
func AccountsReceivableAging2(ctx context.Context, client *generated.Client, name *string) ([]*model.AgingBucket, error) {
	...
	if name != nil && *name != "" {
		nameFilter = fmt.Sprintf("AND LOWER(name) = LOWER('%s')", *name)
	}
	...
	ORDER BY 
		...
		END;
	`, now, now, now, now, currentCompany.ID, nameFilter)
